...
enum ImGuiCond_
{
    ...
    ImGuiCond_FirstUseEver  = 0,   // Set the variable if the object/window has no persistently saved data (no entry in .ini file)
...
struct ImGuiIO
{
    ...
    const char* IniFilename = NULL;                    // = "imgui.ini"    // Path to .ini file. Set NULL to disable automatic .ini loading/saving, if e.g. you want to manually load/save from memory.
    ...
    bool        ConfigWindowsMoveFromTitleBarOnly; //= true;      // [BETA] Set to true to only allow moving windows when clicked+dragged from the title bar. Windows without a title bar are not affected.
